...
error_to_string (VGST_ERROR_LOG error_code, gint index) {
    ...
    case VGST_ERROR_LLP2_NOT_SUPPORTED :
     return "LLP2 use-case is not supported";
    case VGST_ERROR_DRM_BUS_ID_NOT_FOUND :
     return "DRM bus id is not found";
    case VGST_ERROR_GDR_MODE_MISMATCH :
     return "GDR mode is currently supported with LLP1/LLP2 low-delay-p use-cases only";
    case VGST_ERROR_ENTROPY_MODE_NOT_SUPPORTED :
     return "Entropy mode is not supported";
    case VGST_ERROR_MAX_PICTURE_SIZE_NOT_SUPPORTED :
     return "Max picture size is supported with CBR/VBR rate control mode only";
    case VGST_ERROR_RELATIVE_QP_RANGE_MISMATCH :
     return "Relative qp range limitations. It only supports range <-32 to +31>";
    ...
    switch (error) {
      ...
      case VLIB_ERROR_SCD_INVALID_CONFIG:
          return vlib_error_name(VLIB_ERROR_SCD_INVALID_CONFIG);
      case VLIB_ERROR_SCD_CH_NOT_AVAILABLE :
        return vlib_error_name(VLIB_ERROR_SCD_CH_NOT_AVAILABLE);
...
on_deep_element_added (GstBin *bin,
    ...
    if (!g_strcmp0 (klass, "Codec/Decoder/Video/Hardware")) {
      ...
      if (*enable_roi && g_object_class_find_property(G_OBJECT_GET_CLASS(element), "split-input")) {
        g_object_set (G_OBJECT (element),  "split-input",     TRUE,     NULL);
        GST_DEBUG ("Setting split-input to TRUE");
      }
    }
}
...
vgst_print_params (guint index) {
    ...
    if (FALSE == ip_param->raw) {
      ...
      GST_DEBUG ("GDR_mode %u", enc_param->gdr_mode);
      GST_DEBUG ("Low_bandwidth %u", enc_param->low_bandwidth);
      GST_DEBUG ("HLG_SDR_Compatible %u", enc_param->hlg_sdr_compatible);
    }
